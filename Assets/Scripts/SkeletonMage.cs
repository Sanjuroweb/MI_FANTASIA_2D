...
public class SkeletonMage : MonoBehaviour
{
    ...
    private IEnumerator LanzarBola(Vector2 direccionBola, float distancia)
    {
        lanzandoBola = true;
        anim.SetBool("atacando", true);
        yield return new WaitForSeconds(0.45f); //tiempo entre cada bolaMago
        anim.SetBool("atacando", false);
        //53 para mejorar la direccion de la bolaMago
        direccionBola = (player.transform.position - transform.position).normalized * distanciaDeteccionBola;
        direccionBola = direccionBola.normalized;

        //instanciamos la bolaMago en la posición actual del objeto
        //el 3er param es una rotacion poero nosotros le metemos quaternion
        GameObject bolaGO = Instantiate(bolaMago, transform.position, Quaternion.identity);
        //a la bolaMago le pasamos la direccion que hemos definido aqui
        // Se obtiene el componente bolaMago del objeto recién instanciado y se le asigna la dirección de la bolaMago.
        bolaGO.transform.GetComponent<BolaMago>().direccionBola = direccionBola;
        //lo mismo pero con el esqueleto
        // Se obtiene el componente bolaMago del objeto recién instanciado y se le asigna el objeto esqueleto (this.gameObject).
        bolaGO.transform.GetComponent<BolaMago>().esqueleto = this.gameObject;

        // Se aplica una velocidad a la bolaMago multiplicando la dirección por la fuerza de lanzamiento.
        bolaGO.transform.GetComponent<Rigidbody2D>().velocity = direccionBola * fuerzaLanzamiento;
